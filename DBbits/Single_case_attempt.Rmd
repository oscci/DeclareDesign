...
```
This simple dataset illustrates some of the problems in interpreting single case data from a study of this kind. Just on the basis of visual inspection, it looks as if the intervention was effective: the scores increase during the intervention period, and during this period they are higher than all but one of the baseline scores. However, the baseline scores are quite noisy: indeed if we had only sessions 1-3, we would argue that there is an upward trend similar to that seen with intervention. We might be tempted to compare the set of baseline scores with the intervention scores on a one-tailed t-test: if we did that we would find that the t-value was `r round(abs(tparker$statistic,2))` with associated p-value of `r round(tparker$p.value,3)`, and conclude that there was good evidence for effectiveness of the intervention in this individual. However, this approach to analysis and interpretation is misguided for two reasons.  

First, the t-test is only valid if the datapoints are independent. These data all come from the same person, and form a time-series, so they do not meet this assumption. This might seem like a statistician's quibble, but it has serious consequences. If datapoints are not independent then the p-values returned from the t-test are likely to be exaggerated and so give unwarranted confidence in the difference.

Second, even if the statistical comparison between the two intervals was valid, it would only indicate intervention efficacy if there was no other reason for scores to improve over time. The argument here is exactly the same as those used when considering the need for a control group in a randomized controlled trial - without a control group, you can't be sure that the change you observe is due to the intervention, rather than to more general factors, such as the person becoming more familiar with test situation, or having positive expectations from the intervention that makes them more motivated. In the case of children, some improvement may be due just to general maturation, or to impact of concurrent education. 

Several methods can be used to control for such general reasons for improvement:

* If the baseline is flat over a series of measurements, this is generally regarded as reasonable evidence that improvement in the intervention period reflects a genuine treatment effect  

* As well as measuring improvement on a treated outcome, the study can incorporate another outcome measure that is not expected to improve. If improvement is seen only in the treated outcome measure, this gives more confidence in the effect.  

* If a series of cases is studied, then the timing of intervention can be varied across cases. The prediction, if intervention is effective, is that improvement will be more associated with whether or not intervention has occurred, rather than with the specific time point of measurement.  
...
for (m in 1:3){
calder9Am <- filter(calder9A,morpheme==mymorph[m])
calder9Bm<- filter(calder9B,morpheme==mymorph[m])
calder9Cm <- filter(calder9C,morpheme==mymorph[m])
...
calder9Am<-caldermeans(calder9Am)
calder9Bm<-caldermeans(calder9Bm)
calder9Cm<-caldermeans(calder9Cm)
...
plot(1:(endcol-2),calderfile[1,startcol:endcol],xlim=c(1,(endcol-2)),ylim=c(0,1),type='n',xlab='Session',ylab='Outcome score',xaxp = c(1, (endcol-2), (endcol-3)),
     main=mymorph[m])
for (r in 1:4){ #plot individual rows
  mywd <-1
...
par(mfrow=c(1,3))
calderplot(calder9Am)
calderplot(calder9Bm)
calderplot(calder9Cm)
}
