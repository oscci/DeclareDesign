...
# For Calder et al, this might be all the children with DLD in the language units they sampled from, e.g. 150
  ...
  startprob =.3
  buskcut = 1.28 #cutoff for categorising using Busk d
...
M <- declare_model(
  ...
  #We now compare with Busk and Serlin's measure, d2
   ...
   buskcat = ifelse(busk_d2>buskcut,1,0),  #value greater than 1.65 indicates sig increase in score
...
```{r answerA,echo=T}
A <- declare_estimator(
  correctcat ~ 1, model = lm_robust, inquiry = "meancorrcat",label="meancorrcat") + #This is responder/nonresponder categorisation correct
 declare_estimator(
  correctbusk ~ 1, model = lm_robust, inquiry = "meancorrbusk",label="meancorrbusk") #This is categorisation by busk
...
diagnose_design(design) %>% reshape_diagnosis() %>% kable()
