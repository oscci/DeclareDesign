...
M <- declare_model(
  ...
  tau = round(runif(N, min = 3, max = 5),0), # treatment effect - here specified to vary at random for each person from min to max. These values selected to give data that resemble Calder et al (though does depend on proportion of responders as well as max and min)
  ...
  initialset = c(0,1,1,2,2,2,3,3,4,4,5,6,7,8,9,10),
  #Initialset was originally 1:10, but to be realistic, we need to bias initial baseline score towards low values, hence a hand-crafted set. 
  Y1_1 = sample(initialset, N, replace=T), #baseline % correct is random number from initialset
...
A <- declare_estimator(
  correctcat ~ 1, 
  model = lm_robust, 
  inquiry = "correctlyIDed") #This is responder/nonresponder categorisation correct
...
draw_estimates(designA) %>% kable()

diagnose_design(designA) %>% reshape_diagnosis() %>% kable()
