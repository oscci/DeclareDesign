...
```{r specifygeneralparameters,echo=F}
  ...
  mineffect = 3 #used in definition of treatment effect; n items increase in score
...
M <- declare_model(
  ...
  Y2_1 = round(r*Y1_1+sqrt(1-r^2)*U,0), #Y1_1 is baseline measure, Y2_1 is 2nd baseline, correlates with Y1_1 with correlation r
...
M3 <- declare_model(
  ...
  Y1_1 = rbinom(N,10,startprob), #baseline % correct as binomial where startprob is prob of item correct
  Y2_1 = round(r*Y1_1+sqrt(1-r^2)*U,0), #Y1_1 is baseline measure, Y2_1 is 2nd baseline, correlates with Y1_1 with correlation r
  Y3_1 = round(r*Y2_1+sqrt(1-r^2)*U,0), #3rd baseline
  Y1_2 = round(r*Y3_1+sqrt(1-r^2)*U,0), #Y1_2 is first post-intervention measure, correlates with Y3_1
  Y2_2 = round(r*Y1_2+sqrt(1-r^2)*U,0), #Y2_2 is 2nd post-intervention measure, correlates with Y1_2
  Y3_2 = round(r*Y2_2+sqrt(1-r^2)*U,0), #Y3_2 is 3rd post-intervention measure, correlates with Y2_2
  #Now do potential outcomes for post-intervention measures - everyone treated, so Z is 1. 
  Y1_2_Z = Y1_2+Z*tau*responder, #treatment effect added to Y1_2
  Y2_2_Z = Y2_2+Z*tau*responder, #treatment effect added to Y2_2
  ...
  Y2_1 = if_else(Y2_1 > 10, 10, Y2_1),
  Y3_1 = if_else(Y3_1 > 10, 10, Y3_1),
  Y1_2_Z = if_else(Y1_2_Z>10, 10, Y1_2_Z),
  Y2_2_Z = if_else(Y2_2_Z>10, 10, Y2_2_Z),
  Y3_2_Z = if_else(Y3_2_Z>10, 10, Y3_2_Z),
  Y2_1 = if_else(Y2_1 < 0, 0, Y2_1),
...
plot(1:4,yvals,type='b',xaxt = "n",ylab="Mean score",xlab="",ylim=c(0,9),main='Sample plot of results\nResponders in red',lwd=3)
for (i in 1:nrow(mydraw)){
  lines(1:4,mydraw[i,w],type='l',lty=2,col=1+(2*mydraw$correctbusk[i]+mydraw$responder[i]) )
...
#The plot colour codes each person, according to whether they really are a responder or nonresponder, and how they are classifed by the busk criterion. So Misclassified resp is a responder who was misclassified as non-responder
